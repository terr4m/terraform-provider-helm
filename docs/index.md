---
page_title: "Helm Provider - terraform-provider-helm"
subcategory: ""
description: |-
  Helm provider.
---
...
- `registry_config` (String) Path to the _Helm_ registry configuration file. Can be set with the `HELM_REGISTRY_CONFIG` environment variable.
- `repository_cache` (String) Path to the _Helm_ repository cache. Can be set with the `HELM_REPOSITORY_CACHE` environment variable.
- `repository_config` (String) Path to the _Helm_ repository configuration file. Can be set with the `HELM_REPOSITORY_CONFIG` environment variable.
